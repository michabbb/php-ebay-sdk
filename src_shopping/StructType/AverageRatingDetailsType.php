 ...
 * neutral, or negative) for a seller, they also can leave ratings in four areas: item as described, communication, shipping time, and charges for shipping and handling. Users retrieve detailed ratings as averages of the ratings left by buyers. The
 * <b>AverageRatingDetails</b> container is only returned in the <b>GetUserProfile</b> call if the <b>IncludeSelector</b> field is included in the call request and set to <b>FeedbackHistory</b>
 * @subpackage Structs
 */
class AverageRatingDetailsType extends AbstractStructBase
{
     ...
     * @uses \macropage\ebaysdk\shopping\EnumType\FeedbackRatingDetailCodeType::valueIsValid()
     * @uses \macropage\ebaysdk\shopping\EnumType\FeedbackRatingDetailCodeType::getValidValues()
    ...
    public function setRatingDetail(?string $ratingDetail = null): self
    {
        ...
        if (!\macropage\ebaysdk\shopping\EnumType\FeedbackRatingDetailCodeType::valueIsValid($ratingDetail)) {
            throw new InvalidArgumentException(sprintf('Invalid value(s) %s, please use one of: %s from enumeration class \macropage\ebaysdk\shopping\EnumType\FeedbackRatingDetailCodeType', is_array($ratingDetail) ? implode(', ', $ratingDetail) : var_export($ratingDetail, true), implode(', ', \macropage\ebaysdk\shopping\EnumType\FeedbackRatingDetailCodeType::getValidValues())), __LINE__);
        }
